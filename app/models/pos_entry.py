...
class GeneralNote(Base):
    ...
    id = Column(Text, primary_key=True, server_default=text("generate_custom_id('GN')"))
...
class Item(Base):
    ...
    id = Column(Text, primary_key=True, server_default=text("generate_custom_id('PR')"))
    general_note_id = Column(Text, ForeignKey('general_notes.id', ondelete='CASCADE'), nullable=False)
...
class Barcode(Base):
    ...
    id = Column(Text, primary_key=True, server_default=text("generate_custom_id('BR')"))
    general_note_id = Column(Text, ForeignKey('general_notes.id', ondelete='CASCADE'), nullable=False)
    ...
    barcode_products = relationship("BarcodeProduct", back_populates="barcode", cascade="all, delete-orphan")
...
class BarcodeProduct(Base):
    ...
    id = Column(Text, primary_key=True, server_default=text("generate_custom_id('BP')"))
    barcode_id = Column(Text, ForeignKey('barcodes.id', ondelete='CASCADE'), nullable=False)
    ...
    # Relationships
    barcode = relationship("Barcode", back_populates="barcode_products")
