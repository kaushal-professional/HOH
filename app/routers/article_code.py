...
def scan_barcode(
    ...
    # Step 3.5: Validate product is assigned to this store in store_product table
    if store_name:
        store_product_record = db.query(StoreProductFlat).filter(
            StoreProductFlat.product_name == product_name,
            StoreProductFlat.store == store_name
        ).first()

        if not store_product_record:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Product '{product_name}' is not assigned to store '{store_name}'. Please check the store assignment."
            )
...
def lookup_article_by_name(request: ArticleLookupRequest, db: Session = Depends(get_db)):
    ...
    # Step 2.5: Validate product is assigned to this store in store_product table
    if request.store_name:
        store_product_record = db.query(StoreProductFlat).filter(
            StoreProductFlat.product_name == product_name,
            StoreProductFlat.store == request.store_name
        ).first()

        if not store_product_record:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Product '{product_name}' is not assigned to store '{request.store_name}'. Please check the store assignment."
            )
