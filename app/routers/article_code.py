...
# Optional fallback decoder (ZXing Java via pyzxing)
try:
...
# Textract-based OCR extractor (AWS)
try:
    from app.services.barcode_extractor_env import BarcodeExtractor
except Exception:
    # Allow running this file standalone/tests where module path differs
    from barcode_extractor_env import BarcodeExtractor  # type: ignore
...
async def scan_barcode(
    ...
    try:
        ...
        # --------------------------------------------------------------------
        # IMMEDIATELY PASS TO TEXTRACT - NO PREPROCESSING
        # --------------------------------------------------------------------
        print("🔍 IMAGE RECEIVED - Sending to Textract immediately...")
        barcode_text = try_extract_with_textract(bgr)
        print(f"✅ Textract result: {barcode_text if barcode_text else 'None - proceeding to fallbacks'}")

        # --------------------------------------------------------------------
        # FALLBACK METHODS (only if Textract fails)
        # --------------------------------------------------------------------
        ...
        # 2) FALLBACK: Try decoding barcode with pyzxing (ZXing Java)
        if not barcode_text:
            print("⚠️ Textract failed - Trying pyzxing...")
            barcode_text = try_decode_with_pyzxing(bgr, preprocess=preprocess)
            if barcode_text:
                print(f"✅ pyzxing result: {barcode_text}")

        # 3) LAST RESORT: Try decoding with zxing-cpp (keeps existing behavior)
        if not barcode_text:
            print("⚠️ pyzxing failed - Trying zxing-cpp...")
            ...
            if results and len(results) > 0 and results[0].text:
                barcode_text = results[0].text
                print(f"✅ zxing-cpp result: {barcode_text}")

        if not barcode_text:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="No barcode detected (Textract OCR + pyzxing + zxing-cpp). Please ensure the image contains a clear barcode."
            )
...
# ============================================================================
# TEXTRACT + PYZXING FALLBACK HELPERS
# ============================================================================
...
def _extract_barcode_digits_from_textract_result(result: dict) -> Optional[str]:
    """Pick the most likely long numeric string from Textract LINE/WORD output."""
    if not result:
        return None

    texts = []
    for x in result.get("lines", []) or []:
        t = (x.get("text") or "").strip()
        if t:
            texts.append(t)
    if not texts:
        for x in result.get("words", []) or []:
            t = (x.get("text") or "").strip()
            if t:
                texts.append(t)
    ...
    # Prefer longest; break ties by frequency
    candidates_sorted = sorted(candidates, key=lambda x: (len(x), candidates.count(x)), reverse=True)
    return candidates_sorted[0]


def try_extract_with_textract(bgr: np.ndarray) -> Optional[str]:
    """Use AWS Textract to OCR the label and extract the barcode digits."""
    try:
        extractor = BarcodeExtractor(image_array=bgr)
        # Use the new method with debug logging
        barcode_text = extractor.extract_barcode_number_via_textract(use_analyze_document=False)
        if barcode_text:
            return barcode_text
        # Fallback to old method if new one returns None
        result = extractor.send_full_image_to_textract(use_analyze_document=False)
        return _extract_barcode_digits_from_textract_result(result)
    except Exception:
        return None


def try_decode_with_pyzxing(bgr: np.ndarray, preprocess: bool = True) -> Optional[str]:
    """Decode barcode using pyzxing (ZXing Java). Returns text or None."""
    ...
    try:
        img = bgr
        if preprocess:
            # Use the same preprocessing pipeline (grayscale/sharpen) for better decode
            img = preprocess_with_cv2(bgr)
        else:
            img = cv2.cvtColor(bgr, cv2.COLOR_BGR2GRAY)

        with tempfile.TemporaryDirectory() as td:
            tmp_path = str(Path(td) / "scan.png")
            cv2.imwrite(tmp_path, img)

            reader = BarCodeReader()
            decoded = reader.decode(tmp_path) or []

            # decoded is usually a list[dict]
            for r in decoded:
                txt = r.get("parsed") or r.get("raw")
                if txt:
