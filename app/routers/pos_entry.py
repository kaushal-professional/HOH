...
def create_pos_entry(
    ...
    try:
        ...
        for page_data in pos_entry.general_note.barcode_scanned_pages:
            ...
            db.add(barcode)
            db.flush()  # Flush to get the barcode ID
            ...
            for product_data in page_data.products:
                ...
                db.add(barcode_product)
                created_products.append(barcode_product)
                total_products_scanned += 1

            # Attach products to barcode for response
            barcode.barcode_products = created_products
            created_barcodes.append(barcode)

        # Commit all changes
        db.commit()

        # Refresh to get all data including generated IDs and timestamps
        db.refresh(general_note)
        for item in created_items:
            db.refresh(item)
        for barcode in created_barcodes:
            db.refresh(barcode)
            for product in barcode.barcode_products:
                db.refresh(product)
        ...
        return POSEntryResponse(
            ...
            total_barcode_pages=len(created_barcodes),
