...
def download_stock_variance_by_date_range(
    ...
    - **start_date**: Start date inclusive (required, format: YYYY-MM-DD)
    - **end_date**: End date inclusive (required, format: YYYY-MM-DD)

    Returns products within the date range with:
    - **start_date**: Date from open_stock created_at
    - **end_date**: Date from close_stock created_at
    - **product**: Product name
    - **unit_of_measure**: Unit of measure from store_product table
    - **ykey**: Y-Key from store_product table
    - **open_qty**: Opening quantity
    ...
    # Convert date to datetime for repository query (start of day to end of day)
