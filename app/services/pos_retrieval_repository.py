...
class POSRetrievalRepository:
    ...
    @staticmethod
    def _calculate_gst_percentage(price: Decimal, gst: Decimal) -> Decimal:
        """Calculate GST percentage from price and GST amount"""
        if price and price > 0 and gst is not None:
            return (gst / price) * Decimal('100')
        return Decimal('0')

    @staticmethod
    def _calculate_price_including_tax(price: Decimal, gst_percentage: Decimal) -> Decimal:
        """Calculate price including tax from price and GST percentage"""
        if price is not None:
            return price * (Decimal('1') + gst_percentage / Decimal('100'))
        return Decimal('0')
    ...
    def get_pos_data(
        ...
        for row in results:
            ...
            # Calculate GST percentage from price and GST amount
            price_excl = row.price_excluding_tax or Decimal('0')
            gst_amount = row.taxes or Decimal('0')
            gst_percentage = POSRetrievalRepository._calculate_gst_percentage(price_excl, gst_amount)

            # Calculate price including tax from price and GST percentage
            price_incl_tax = POSRetrievalRepository._calculate_price_including_tax(price_excl, gst_percentage)
