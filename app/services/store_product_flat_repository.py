...
class StoreProductFlatRepository:
    ...
    @staticmethod
    def bulk_upsert(db: Session, entries: List[StoreProductFlatCreate]) -> dict:
        """
        Bulk create or update store product entries.
        If (ykey, store, state) combination exists, update product_name.
        ...
        for idx, entry in enumerate(entries):
            try:
                # Check if entry exists with matching (ykey, store, state)
                existing = db.query(StoreProductFlat).filter(
                    and_(
                        StoreProductFlat.ykey == entry.ykey,
                        StoreProductFlat.store == entry.store,
                        StoreProductFlat.state == entry.state
                    )
                ).first()
        ...
        try:
            ...
            return {
                ...
                "updated_count": 0,
                "failed_count": len(entries),
                "errors": [{"error": f"Bulk commit failed: {str(e)}"}]
            }
